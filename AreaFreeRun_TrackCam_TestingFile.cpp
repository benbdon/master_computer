...
#define WIDTH		924				// 4-8192 in increments of 4 -> TrackCam max of 1024
#define HEIGHT		924				// 1-8192 in increments of 1 -> TrackCam max of 1024
...
HANDLE ConfigureSerialPortGRBL(HANDLE m_hSerialComm, const wchar_t* m_pszPortName) {
...
*/
	...
	// Serial communication using port COM13
	const wchar_t* m_pszPortNameGRBL = ComPortGRBL;
	...
	if( ext == "txt.") {  //backwards .txt
		...
		if( myFile.is_open()) {
			...
			while( LinesRead < LinesInTextFile ) {
				...
				/*
				if( pMem0 != Fg_AllocMemEx(fg,totalBufSize,nr_of_buffer) ) {
					printf("D\n\n");
					pMem0 = Fg_AllocMemEx(fg,totalBufSize,nr_of_buffer);
				}
				printf("E\n\n");
				*/
				...
				int topFrames = ((NUMIMAGES_Side/INTEGER_MULTIPLE)-1);
				...
				if( MemSet == false ){
					...
					buf[i] = new unsigned char[width*height];
					}
					MemSet = true;
				}

				/*
				for(int i=iLast;i< (iLast + topFrames);i++)
				{
					printf("i: %d\n\n", i);
					//buf[i] = (unsigned char *) malloc(width*height);
					buf[i] = new unsigned char[width*height];
				...
				*/
				...
				while( (strcmp(inet_ntoa(SenderAddr.sin_addr),CorrectIP) ) || (strcmp(RecvBuf,"Start sequence AVI.") )) { // 17-04-17 by Mengjiao
					iResult = recvfrom(RecvSocket, RecvBuf, BufLen, 0, (SOCKADDR *) & SenderAddr, &SenderAddrSize);
					if (iResult == SOCKET_ERROR) {
						wprintf(L"recvfrom failed with error %d\n", WSAGetLastError());
						//printf("B\n");
				...
				*/
				...
				while (PPOD_ERROR >= errorThreshold) {
					...
					iResult = recvfrom(RecvSocketPPOD, RecvBufPPOD, BufLen, 0, (SOCKADDR *) & SenderAddr, &SenderAddrSize);
					if (iResult == SOCKET_ERROR) {
						wprintf(L"recvfrom failed with error %d\n", WSAGetLastError());
					}
					...
					while( strcmp(inet_ntoa(SenderAddr.sin_addr),CorrectIP) ) { //17-04-17 by Mengjiao
						iResult = recvfrom(RecvSocketPPOD, RecvBufPPOD, BufLen, 0, (SOCKADDR *) & SenderAddr, &SenderAddrSize);
						if (iResult == SOCKET_ERROR) {
							wprintf(L"recvfrom failed with error %d\n", WSAGetLastError());
						}
				...
				//printf("NUMIMAGES_Side: %d \r\n", NUMIMAGES_Side);
				//printf("INTEGER_MULTIPLE: %d \r\n", INTEGER_MULTIPLE);
				//printf("Control: %d \r\n", ((NUMIMAGES_Side/INTEGER_MULTIPLE)-1));
			
					// Get last frame number - non-blocking function call
					
				printf("--lastPicNr = %d \r\n",Fg_getLastPicNumber(fg,nCamPort)); // 17-02-12 by Mengjiao
				...
				while( lastPicNr < ((NUMIMAGES_Side/INTEGER_MULTIPLE)-1) ) {			// Subtracted 1 (everywhere else too) to avoid missing the last lastPicNr frequently - don't know why it does
					...
					// =============================================================================================================
					//Move DOD out of way of overhead camera
					// =============================================================================================================
					if( lastPicNr == 1 && CAM_MOVE == 1) {
						printf("Moving out of frame.\r\n");
						//PositionRelative(m_hSerialCommGRBL, GCODEmessage, -50, 60);
						PositionAbsoluteNoRead(m_hSerialCommGRBL, GCODEmessage, camX, camY);
						//printf("done");
					}
					
					//testing capture frames
					//printf("tempCounter: %d \r\n", tempCounter);
					printf("--lastPicNr: %d \r\n", Fg_getLastPicNumber(fg,nCamPort));
					...
					printf("lastPicNr = %d \r\n",lastPicNr); // 17-02-12 by Mengjiao

					// Store pointer to im age in buf[]
					//printf("Frame location: %d \r\n", iLast + tempCounter);
					FrameNumbers[iLast + tempCounter] = lastPicNr;
					buf[iLast + tempCounter] = (unsigned char *) Fg_getImagePtrEx(fg,lastPicNr,0,pMem0);
					tempCounter++;
				...
				if( IDENTIFIER == 'S' ) {
					...
					sprintf(filename, "TrackCam_%c_%d_%dHz_%fDelayTime_%dFPS_%dPulseTime.avi", IDENTIFIER, SAVEDSIGNAL, FREQ, DELAYTIME, (FPS_Side/INTEGER_MULTIPLE), PULSETIME);
				}
				...
				writer2.open(filepath, codec, fps, FrameSize, isColor);
				...
				if (writer2.isOpened()) 
				{
					...
					for(int i=iLast;i<iLast + ((NUMIMAGES_Side/INTEGER_MULTIPLE)-1);i++) {
				...
				//free(buf); // breaks code?
				//free(a);   // breaks code?
				writer2.release();

				iLast = iLast + ((NUMIMAGES_Side/INTEGER_MULTIPLE)-1);
				//printf("iLast = %d \r\n",iLast);// 17-02-12 by Wenbo

	/*			// =================================================================================================================
				// Receive datagram from Machine B to turn off the ExSync trigger - 17-02-26 by Mengjiao
				// =================================================================================================================

				// Call the recvfrom function to receive datagrams from Machine B
				wprintf(L"Waiting for Machine B to turn off the ExSync...\n");
				...
				CorrectIP = "129.105.69.220";
				wprintf(L"sender address = %d\n", SenderAddr.sin_addr);
				wprintf(L"CorrectIP = %d\n", CorrectIP);
				wprintf(L"RecvBuf = %s\n",RecvBuf);
				while( (strcmp(inet_ntoa(SenderAddr.sin_addr),CorrectIP)) || (strcmp(RecvBuf,"Sequence AVI captured."))) {
	...
	*/
				...
				// =================================================================================================================
				nExSyncEnable		= 0;
				printf("Turning ExSync Enable OFF since finished receiving triggers. \r\n");// 17-02-12 by Mengjiao
				if(Fg_setParameter(fg,FG_EXSYNCON,&nExSyncEnable,nCamPort)<0)		{ status = ErrorMessageWait(fg); return status;}
				...
				if( PPOD_RESET == 1 ) {
					...
					// Create message to send
					memset(&SendBuf[0], 0, sizeof(SendBuf));		
					sprintf(UDPmessage,"%c %d %d %d %d %d %d %d", IDENTIFIER, 0, 0, 0, 0, 0, 0, 0);		
					strcpy(SendBuf,UDPmessage);
				...
				// =====================================================================================================
				// Determine if more lines to be read in file, or at the end of file
				// =====================================================================================================
				if( LinesRead < LinesInTextFile ) {
					RunFlag = 1;
					printf("More lines to be read.\r\n\n");
				}
				else {
					RunFlag = 0;
					printf("Last line in file.\r\n\n");
					
					printf("Moving home.\r\n");
					HomeNoRead(m_hSerialCommGRBL, GCODEmessage);
					printf("done");
				...
				/*
				if (iResult != SOCKET_ERROR){		// 17-02-12 by Wenbo
					wprintf(L"Sent RunFlag successfully...\n");
				...
				*/
				...
				//while( (strcmp(inet_ntoa(SenderAddr.sin_addr),CorrectIP) != 0) && (ntohs(SenderAddr.sin_port) != CorrectPort) ) {
				//wprintf(L"RecvBuf = %s\n",RecvBuf);
				while( (strcmp(inet_ntoa(SenderAddr.sin_addr),CorrectIP) || strcmp(RecvBuf,"Message received."))) {
					iResult = recvfrom(RecvSocket, RecvBuf, BufLen, 0, (SOCKADDR *) & SenderAddr, &SenderAddrSizeA);
					wprintf(L"RecvBuf = %s\n",RecvBuf);
					if (iResult == SOCKET_ERROR) {
						wprintf(L"recvfrom failed with error %d\n", WSAGetLastError());
						memset(&RecvBuf[0], 0, sizeof(RecvBuf));
	...
	*/
    ...
    iResult = closesocket(RecvSocketPPOD);
    if (iResult == SOCKET_ERROR) {
        wprintf(L"closesocketppod failed with error %d\n", WSAGetLastError());
        return 1;
    }	
