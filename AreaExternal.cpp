...
int main(int argc, char* argv[], char* envp[]){
	...
	int nr_of_buffer	=	45000;		// Number of memory buffer
	...
	int		nTriggerMode		= ASYNC_TRIGGER; //2: Extern Trigger 
	int	    nExposureInMicroSec	= 20;
	int		nTrgSource			= 0;
	...
	// MAIN LOOP
	int lastPicNr = 0;
	while((lastPicNr = Fg_getLastPicNumberBlockingEx(fg,lastPicNr+1,nCamPort,10,pMem0))< NUMIMAGES) {
		if(lastPicNr <0){
			status = ErrorMessageWait(fg);
