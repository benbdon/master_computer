...
#define ComPortGUI	   L"COM6"		// Serial communication port - neccessary to add L in front for wide character
...
*/
	...
	// =====================================================================================================================
	// Configuration of serial port: COM6 (virtual COM port pair 5/6 - com0com program) to talk with TestingGUI (COM5)
	// =====================================================================================================================
	...
	while( RunFlag == 1 ) {
		...
		// =================================================================================================================
		// Receive serial command from COM5 with current testing parameters
		// =================================================================================================================
		...
		printf("Waiting to read from GUI - COM6.\r\n");
		TestingGUIRead = ReadSerialPort (m_hSerialCommGUI);
		...
		sscanf(TestingGUIRead.c_str(), "%d%*c %d%*c %d%*c %d%*c %c%*c %d%*c %d%*c %d%*c %d%*c %d%*c %f%*c %d", &FREQ, &VERT_AMPL, &HORIZ_AMPL, &PHASE_OFFSET, &IDENTIFIER, &SAVEDSIGNAL, &FPS_Side, &NUMIMAGES_Side, &INTEGER_MULTIPLE, &PULSETIME, &DELAYTIME, &PPOD_RESET);
		...
		if( IDENTIFIER == 'S' ) {
			...
			sprintf(UDPmessage,"%c, %d, %d, %d, %d, %d, %d, %d, %f", IDENTIFIER, FREQ, VERT_AMPL, HORIZ_AMPL, PHASE_OFFSET, FPS_Side, NUMIMAGES_Side, PULSETIME, DELAYTIME);
		}
		...
		if( IDENTIFIER == 'S' ) {
			...
			sprintf(filename, "TrackCam_%c_%dHz_%dA_%dA_%03dDPhase_%fDelayTime_%dFPS_%dPulseTime.avi", IDENTIFIER, FREQ, HORIZ_AMPL, VERT_AMPL, PHASE_OFFSET, DELAYTIME, (FPS_Side/INTEGER_MULTIPLE), PULSETIME);
		}
		...
		//free(buf);   //breaks code when running
		//free(a);     //breaks code when running
		writer2.release();
